...
public class RecycleShop extends HandlerBase{
  private static Config conf = new Config(Main.getDataPath()+"/回收商店.yml",Config.YAML);
  @SuppressWarnings("unchecked")
	public static void send(Player player)//给玩家发分类页面
  {
    MultiOption ui = new MultiOption("§r§l回收商店");
  ...
  public void handle(String data,Player player)//处理分类页面
  {
    ...
    for(Map.Entry<String,Object> group : conf.getAll().entrySet())
    {
    	if(String.valueOf(i).contentEquals(data))
    	{
    		...
    		RecycleClassifier.send(player, (ArrayList<Map<String, Object>>)list.get("列表"), group.getKey());
    	}
    	i++;
    }
  }
